...
async function generateOGImage() {
  ...
  // Ensure high-efficiency WOFF2 fonts exist for zero-latency startup
  const fontsToCompress = [
    { src: 'public/fonts/gotham-medium.otf', out: 'public/fonts/gotham-medium.woff2' },
    { src: 'public/fonts/gotham-regular.ttf', out: 'public/fonts/gotham-regular.woff2' },
  ];
  for (const f of fontsToCompress) {
    const sPath = path.join(rootDir, f.src);
    const oPath = path.join(rootDir, f.out);
    if (fs.existsSync(sPath) && !fs.existsSync(oPath)) {
      const raw = fs.readFileSync(sPath);
      const comp = await wawoff2.compress(raw);
      fs.writeFileSync(oPath, comp);
      console.log(`Compressed font to ${f.out} (${comp.length} bytes)`);
    }
  }
}
