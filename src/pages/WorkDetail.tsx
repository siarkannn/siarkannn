...
export function WorkDetail({ slug: propSlug }: { slug?: string } = {}) {
  ...
  useEffect(() => {
    // Defer preloading until page transition animation has completed (1000ms)
  ...
  return (
    <div key={displayProject.slug} className="min-h-screen bg-black text-white flex flex-col justify-between">
      <div className="px-8 min-[390px]:px-9 sm:px-10 md:px-12 pt-[86px] min-[390px]:pt-[88px] sm:pt-[90px] md:pt-24 pb-8 select-none flex-1 transition-[padding] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_2.7fr] lg:grid-cols-[1fr_2.85fr] xl:grid-cols-[1fr_3fr] gap-6 min-[390px]:gap-7 md:gap-10 lg:gap-14 items-start transition-[gap] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]">
          ...
          >
            ...
            <div key={`${displayProject.slug}-title`} className="mb-7 min-[390px]:mb-8 md:mb-10 text-left">
              <h1 className={`font-bold leading-[1.14] md:leading-[1.08] font-['Gotham_Local'] mb-1 min-[390px]:mb-[3px] md:mb-[2px] tracking-[-0.02em] text-white text-left ${
                displayProject.slug.includes("sony-filmmaking-experience")
                  ? "text-[18px] min-[360px]:text-[19.5px] min-[390px]:text-[21.5px] sm:text-[24px] md:text-[44px] lg:text-[47px] xl:text-[48px] whitespace-nowrap md:whitespace-normal"
                  : "text-[23px] min-[390px]:text-[24px] sm:text-[26px] md:text-[44px] lg:text-[47px] xl:text-[48px] break-words [overflow-wrap:anywhere]"
              }`}>
                {displayProject.slug.includes("sony-filmmaking-experience") ? (
                  <>
                    <span className="md:hidden">{displayProject.title}</span>
                    <span className="hidden md:inline">
                      Sony Filmmaking<br />Experience
                    </span>
                  </>
                ) : displayProject.slug.includes("ayubyayulestariofficial") ? (
                  <>
                    <span className="md:hidden">{displayProject.title}</span>
                    <span className="hidden md:inline">
                      Ayubyayu<br />lestariofficial
                    </span>
                  </>
                ) : displayProject.slug.includes("immateurplayground") ? (
                  <>
                    <span className="md:hidden">{displayProject.title}</span>
                    <span className="hidden md:inline">
                      Immateur<br />playground
                    </span>
                  </>
                ) : (
                  displayProject.title
          ...
          {/* Right: images */}
          <div className="order-1 md:order-2 w-full space-y-2 md:pt-20 lg:pt-24 xl:pt-28">
      ...
      <div className="w-full flex items-center justify-between px-8 min-[390px]:px-9 sm:px-10 md:px-12 pt-10 md:pt-12 pb-44 sm:pb-48 md:pb-36 lg:pb-40 select-none transition-[padding] duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]">
        {prevProject ? (
          <button
            onClick={handlePrev}
