...
export function ShowreelVideo({
  ...
  useEffect(() => {
    ...
    // On iOS WKWebView / Instagram in-app browser, listening on touchstart & touchend with capture phase
    // guarantees immediate video start upon the first screen tap or scroll gesture.
    const interactionEvents = ["touchstart", "touchend", "click", "pointerdown"] as const;
    const onUserInteraction = () => {
      const v = videoRef.current;
      if (v && !v.error && v.paused) {
        attemptPlay();
      }
    };
